...
!function (e, t) {
    ...
    var s, l, u, c, d,
        ...
        }, v = {}, h = [], o = 0, n = !1, i = {}, a = !1, y = /.+\.(gif|jpe?g|png|webp)/i, w = {}, k = [], r = null,
        x = function (e) {
    ...
    return [].forEach || (Array.prototype.forEach = function (e, t) {
        ...
        run: function K(e, t) {
            return v.transforms = function n() {
                ...
                (s = J("div")).setAttribute("role", "dialog"), s.id = "baguetteBox-overlay", document.getElementsByTagName("body")[0].appendChild(s), (l = J("div")).id = "baguetteBox-slider", s.appendChild(l), (u = J("button")).setAttribute("type", "button"), u.id = "previous-button", u.setAttribute("aria-label", "Previous"), u.innerHTML = v.svg ? f : "&lt;", s.appendChild(u), (c = J("button")).setAttribute("type", "button"), c.id = "next-button", c.setAttribute("aria-label", "Next"), c.innerHTML = v.svg ? g : "&gt;", s.appendChild(c), (d = J("button")).setAttribute("type", "button"), d.id = "close-button", d.setAttribute("aria-label", "Close"), d.innerHTML = v.svg ? p : "&times;", s.appendChild(d), u.className = c.className = d.className = "baguetteBox-button", function t() {
                    var e = v.passiveEvents ? {passive: !0} : null;
...
$(document).ready(function () {
    $('.drop-down-list__element>div').hide();
    $('.drop-down-list__element>p').click(function () {
        $(this).next().slideToggle('fast');
        $(this).toggleClass('active');
    });

    $('.drop-down-list__elem>ul').hide();
    $('.drop-down-list__elem>p').click(function () {
        $(this).next().slideToggle('fast');
        $(this).toggleClass('active');
    });

    if (window.location.hash.length > 0) {
        $(window.location.hash + ' .drop-down-list__text').slideToggle('fast');
        $(window.location.hash + '.drop-down-list__elem>p').addClass('active');
    }
});

// --- HAMBURGER ---

var hamburger = document.getElementsByClassName('menu-hamburger');
var menu = document.getElementById('mobile-menu');
for (var i = 0; i < hamburger.length; i++) {
    hamburger[i].addEventListener('click', function () {
        if (menu.classList.contains('menu-hamburger__content--show')) {
            menu.classList.remove('menu-hamburger__content--show');
        } else {
            menu.classList.add('menu-hamburger__content--show');
...
// --- POPUP ---
$('.box__popup__button').click(function () {
    var popupSelector = $(this).attr('href');
    $(popupSelector).addClass('overlay--show');
});
